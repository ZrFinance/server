...
class Picture(object):
    def __init__(self, text_str, size, background):
        '''
        text_str: 验证码显示的字符组成的text_str字符串
        size:  图片大小
        background: 背景颜色
        '''
    ...
    def create_text(self, font_type, font_size, font_color, font_num, start_xy):
        ...
        a=" ".join(random.sample(self.text_list, font_num))
        self.draw.text(start_xy, a, font=font, fill=font_color)
        return a
...
def GetPicVerCode():
    ...
    vercode=pic.create_text("simsun.ttc", 24, (0, 0, 205), 5, (7, 7))
    pic.opera()
    name=str(uuid.uuid1())
    filename=os.path.join(settings.PICVERCODE_PATH,"{}.jpg".format(name))
    url="/images/picvercode/{}.jpg".format(name)
    pic.img.save(filename)

    return vercode,url,name